...
func NewRouter(cfg config.Config, logger *slog.Logger, authService *auth.Service, phaseOne *inventory.Service, phaseTwo *procurement.Service, phaseThree *ocr.Service, readyCheck func(context.Context) error) http.Handler {
	...
	mux.HandleFunc("POST /api/v1/operator/master-data/scopes", handlers.UpsertDeviceScope)
	...
	mux.HandleFunc("GET /api/v1/admin/master-data/systems", handlers.ScopeSystems)
	mux.HandleFunc("POST /api/v1/admin/master-data/systems", handlers.UpsertScopeSystem)
	mux.HandleFunc("DELETE /api/v1/admin/master-data/systems/{key}", handlers.DeleteScopeSystem)
	...
	mux.HandleFunc("POST /api/v1/operator/reservations/import/preview", handlers.ReservationImportPreview)
	mux.HandleFunc("POST /api/v1/operator/reservations/import", handlers.ReservationImportApply)
	mux.HandleFunc("POST /api/v1/operator/reservations/allocations/import/preview", handlers.AllocationImportPreview)
	mux.HandleFunc("POST /api/v1/operator/reservations/allocations/import", handlers.AllocationImportApply)
	...
	mux.HandleFunc("POST /api/v1/inventory/operations/{operation}/import/preview", handlers.InventoryOperationImportPreview)
